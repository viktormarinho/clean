...
pub fn get_config() -> EinerConfig {
    ...
    let config: EinerConfig = toml::from_str(&file_str).expect_or_err("O arquivo einer.toml possui sintaxe incorreta. Campos esperados no einer.toml atualmente: env_dirs, open_ports ");
